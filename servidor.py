...
while True:
    ...
    cursor.execute('SELECT matricula, numero_cartao, sala, hora_inicio, dia_semana.id as dia FROM professor, cartao, sala, horario, dia_semana '
                   'WHERE numero_cartao = ? OR matricula = ? AND '
                   'sala = ? AND hora_inicio <= ? AND dia = ?', (professor, professor, sala, hora.strftime("%H:%M:%S"), dia.isoweekday()))
